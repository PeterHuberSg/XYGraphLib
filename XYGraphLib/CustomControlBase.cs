...
namespace XYGraphLib {
  ...
  /// MeasureOverride and ArrangeOverride but 
  /// </summary>
  public abstract class CustomControlBase: Control {
    ...
    Size measureRequestedSize;
      ...
      double borderPaddingWith, borderPaddingHeight;
      MeasureContentSize = MeasureContentOverride(removeBorderPaddingSpace(constraint, out borderPaddingWith, out borderPaddingHeight));
...
//      double borderPaddingWith, borderPaddingHeight;
//      Size lessBorderPaddingSize = removeBorderPaddingSpace(arrangeBounds, out borderPaddingWith, out borderPaddingHeight);
...
//                              VerticalAlignment="{TemplateBinding VerticalContentAlignment}" />
    ...
    Size arrangeRequestedSize;
      ...
      if (arrangeBounds.Width<DesiredSize.Width-Margin.Left - Margin.Right || arrangeBounds.Height<DesiredSize.Height - Margin.Top - Margin.Bottom) {
        TraceWpf.Line(this, "doArrangeOverride(): arrangeBounds " + arrangeBounds + " should be equal or bigger than DesiredSize " + 
    ...
    //private void verifyIsPositive(double value, string valueName) {
    //  if (double.IsNaN(value)) throw new Exception("Custom Control ArrangeContentOverride(): " + valueName + " " + value + " cannot be NAN (not a number).");
    //  if (value<0) throw new Exception("Custom Control ArrangeContentOverride(): " + valueName + " " + value + " cannot be negative.");
    //  if (double.IsInfinity(value)) throw new Exception("Custom Control ArrangeContentOverride(): " + valueName + " " + value + " cannot be infinite.");
    //}
