...
namespace XYGraphLib {
  ...
  /// </summary>
  public partial class LegendYWindow: TestbenchWindow {
    ...
    public static void Show(Window ownerWindow) {
      ShowProtected( () => new LegendYWindow(), ownerWindow);
    }
    ...
    Action testFunc(double height, double minFullValue, double maxFullValue, double displayValue, double displayRange, double[] labels) {
      ...
      return () => {
        if (labels.Length!=TestLegendYTraced.LabelValues.Length) {
          System.Diagnostics.Debugger.Break();
          throw new Exception("There should be " + labels.Length + " legends, but there were " + TestLegendYTraced.LabelValues.Length + ".");
        } else {
